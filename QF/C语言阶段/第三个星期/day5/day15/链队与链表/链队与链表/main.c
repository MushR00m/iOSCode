...
ListNode *ListSearchValue(LinkList head, dataType a)
{
    ListNode *p;
    p=head->next;
    while (p) {
...
BOOL  InsertLinkNode(LinkList head, dataType a, int i)
{
    ListNode *p;
    p=head;
    int j=0;
...
BOOL  DeleteLinkNode(LinkList head,int i)
{
    ListNode *p;
    p=head;
    int j=0;
...
int main(int argc, const char *argv[])
{
    ...
    //head =CreateListHead();
    head = CreateListRear();
    //InsertLinkNode(head, '*', 5);
    DeleteLinkNode(head,5);
    //p=ListSearchValue(head, 'a');
    p=ListSearch(head, 5);
    if (p) {
        printf("%c ", p->data);
    }

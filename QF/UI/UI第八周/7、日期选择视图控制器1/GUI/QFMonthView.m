...
- (void)dealloc
{
    ...
    for (int i=0; i<31; i++)
    {
        [dayButtonArray[i] release];
        dayButtonArray[i] = nil;
    }
    
    [_selectedDayImageView release];
    _selectedDayImageView = nil;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        for (int i=0; i<31; i++)
        {
            ...
            dayButtonArray[i].tag = i;
            
            [dayButtonArray[i] addTarget:self action:@selector(dayButtonClick:) forControlEvents:UIControlEventTouchUpInside];
        }
        
        
        _selectedDayImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"selectedDay.png"]];
    }
    return self;
}
...
{
    NSLog(@"setSelectedDay day = %d", day);
    if (day < 0 || day > 31)
    {
        [_selectedDayImageView removeFromSuperview];
    }
    else
    {
        NSLog(@"dayButtonArray[day-1] = %@", dayButtonArray[day-1]);
        _selectedDayImageView.frame = dayButtonArray[day-1].frame;
        [self addSubview:_selectedDayImageView];
    }
}

- (void)dayButtonClick:(UIButton *)dayButton;
{
    [self setSelectedDay:dayButton.tag+1];
    
    //这里是调用QFDatePickerController类里面的方法
    [self.delegate  didFinishPickingWithYear:_year month:_month day:dayButton.tag+1];
}
...
- (void)setYear:(int)year month:(int)month
{
    ...
    for (int i=0; i<31; i++)
    {
        ...
        {
            [dayButtonArray[i] setTitle:@"今天" forState:UIControlStateNormal];
            [dayButtonArray[i] setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
        }
        else
        {

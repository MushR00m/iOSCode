...
#define PACKET_TYPE_HEART_BEAT 0x00
#define PACKET_TYPE_FILE_DOWNLOAD_REQUEST 0x01
#define PACKET_TYPE_FILE_DOWNLOAD_RESPONSE 0x02
#define PACKET_TYPE_FILE_DATA 0x03
...
typedef struct st_heart_beat
{
    uint8_t   type;//包得类型
    uint32_t  length;//包得长度（整个包得长度、包含包头）
    uint32_t  number;//心跳次数
} ST_HEART_BEAT;

...
int openListenSocket(uint16_t port)
{
    ...
    severAddr.sin_addr.s_addr = INADDR_ANY;//告诉iOS系统 发往本机上所有数据我都可以接受（不管哪一张网卡上得数据）
 ...
 *  @return 大于等于表示创建成功、否则失败。
 */
int openClientSocket(const char *ipstr, uint16_t port)
{
...
ssize_t readN_timeout(int fildes, void *buf, size_t nbyte, long timeoutSec)
{
    ...
    //只有当还有剩下的字节数没有读 同时 还有剩下的时间才进入循环
    while (nbyte > offset && timeoutSec > 0)
    {
        FD_ZERO(&readFdset);
        ...
        int ret = select(fildes+1, &readFdset, NULL, NULL, &timeout);
        ...
        ssize_t nbytes = read(fildes, buf+offset, (nbyte - offset));
        if (nbytes < 0)
        {
            return -1;
        }
...
ssize_t writeN_timeout(int fildes, const void *buf, size_t nbyte, long timeoutSec)
{
    ...
    while (nbyte > offset && timeoutSec > 0)
    {
        ...
        int ret = select(fildes+1, NULL, &writeFdset, NULL, &timeout);
        ...
        ssize_t nbytes = write(fildes, buf+offset, nbyte-offset);
        if (nbytes < 0)
        {
            return -1;
        }

...
@interface CompletionTarget : NSObject


- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo;

@property(assign, readwrite, nonatomic)BOOL flag;
@property(retain, readwrite, nonatomic)NSCondition *condition;

@end

@implementation CompletionTarget

- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo
{
    [self.condition lock];
    
    self.flag = YES;//照片保存完成
...
+ (void)synchronousWriteToSavedPhotosAlbum:(UIImage *)image
{
    NSCondition  *condition = [[NSCondition alloc] init];
    
    CompletionTarget  *completionTarget = [[CompletionTarget alloc] init];
    
    completionTarget.condition = condition;
    completionTarget.flag = NO;
    
    
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        
        UIImageWriteToSavedPhotosAlbum(image, completionTarget, @selector(image:didFinishSavingWithError:contextInfo:), nil);
    });
    
    
    
    [condition lock];
    while (!completionTarget.flag)
    {
        [condition wait];
    }
    [condition unlock];

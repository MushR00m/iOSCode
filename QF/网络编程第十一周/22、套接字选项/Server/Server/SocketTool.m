...
//socket：你要设置属性的套接字 level：套接字选项的级别 option_value：选项值的首地址 option_len：选项值变量所占用的长度。
...
int openListenSocket(uint16_t port)
{
     ...
     */
    int opt = 1;
    ret = setsockopt(listenSocketfd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
    if (ret < 0)
    {
        perror("setsockopt SOL_SOCKET SO_REUSEADDR err");
        close(listenSocketfd);
        return -1;
    }
    
    int sendBugSize = 1024 * 100;
    ret = setsockopt(listenSocketfd, SOL_SOCKET, SO_SNDBUF, &sendBugSize, sizeof(sendBugSize));
    if (ret < 0)
    {
    ...
    int revBugSize = 1024 * 100;
    ret = setsockopt(listenSocketfd, SOL_SOCKET, SO_RCVBUF, &revBugSize, sizeof(revBugSize));
    if (ret < 0)
    {
        perror("setsockopt err");
        close(listenSocketfd);
